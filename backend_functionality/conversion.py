...
for frequency in trimmed_frequency:
    ...
    nperseg = min(len(frequency), n_fft)
...
midi_note = [(int(12 * math.log(freq / 440.0) / math.log(2)) + 69) for freq in filtered_frequency_list]
...
midi_time = [(beat_times[i] - beat_times[i-1]) * 60 / tempo for i in range(1, len(beat_times))]
...
for note, time in zip(midi_note, midi_new_time):
    if note > 0:
        ...
        message_on = Message("note_on", note=note, velocity=velocity, time=int(round(time * 1000)))
        message_off = Message("note_off", note=note, velocity=velocity, time=int(round(time * 1000)))
